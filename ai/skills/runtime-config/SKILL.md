...
Read operator-tunable values through `XFlickrConfig`, not `env()` in application code.
...
- Linked to connections via `app_profile` on `xflickr_connections` when `FlickrService::connection(..., appProfile: 'main')` is called
